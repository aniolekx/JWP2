...
def start_piper():
    ...
    log_file = open('piper_logs.txt', 'a')  # Append mode for logging
    piper_command = ['./piper', '--model', 'en_GB-cori-medium.onnx', '--output-raw']
...
def play_audio(pipe):
    """Plays audio from the given pipe using aplay."""
    aplay_process = subprocess.Popen(['aplay', '-r', '22050', '-f', 'S16_LE', '-t', 'raw'], stdin=pipe)
    aplay_process.wait()
